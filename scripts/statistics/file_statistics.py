...
from scripts.statistics.functions_for_code_maat import *
import os
import csv
...
class FileStatistics:
    def __init__(self, db_path="data_GH_projects/databases/ghbpmn.db",
                 ...
                 code_maat_path="C:\\Users\\viktor\\Documents\education\\bachelorarbeit\\code-maat\\target\\code-maat-"
    ...
    def compute_file_statistics(self):
        query1 = "SELECT DISTINCT login, project_name FROM " + self.table_result_files + ";"
        ...
        for repo in repo_list:
            ...
            if os.path.exists(repo_path):
                query2 = "SELECT path_bpmn_file FROM " + self.table_result_files + \
                         " WHERE login='" + repo[0] + \
                    ...
                    if line_list[0].lower().find("bpmn") != -1:
                        ...
                        for bpmn_file in bpmn_file_list:
                            if git_file_path == bpmn_file[0]:
                                query3 = "UPDATE " + self.table_result_files + " SET n_authors='" + line_list[1] + \
                                         "', n_revs='" + line_list[2] + \
                ...
                for line_list in reader:
                    if line_list[0].lower().find("bpmn") != -1:
                        ...
                        for bpmn_file in bpmn_file_list:
                            if git_file_path == bpmn_file[0]:
                                query4 = "UPDATE " + self.table_result_files + " SET age_months='" + line_list[1] + \
                                         "' WHERE path_bpmn_file='" + bpmn_file[0] + "';"
    ...
    def find_bpmn_files_from_xml(self):
        ...
        query = "SELECT login, project_name, path_bpmn_file FROM " + self.table_result_files + \
        ...
        for (login, project_name, rel_file_path) in all_paths:
            ...
            if os.path.exists(file_path):
                if open(file_path, encoding="utf8", errors='ignore').read().find(bpmn_schema) != -1:
                    query = "INSERT INTO " + self.table_result_bpmn + " VALUES ('" + rel_file_path + "');"
